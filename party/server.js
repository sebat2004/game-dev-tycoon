...
function randomSpawnDelay() {
    return (Math.floor(Math.random() * (MAX_SPAWN_INTERVAL - MIN_SPAWN_INTERVAL + 1)) + MIN_SPAWN_INTERVAL) * 1000;
}
...
const BUG_GENERATION_PROMPT = `You are a Python coding challenge generator for a game. Generate a short Python code snippet (10-25 lines) that contains exactly ONE bug. The bug should be solvable in under 60 seconds by a competent programmer.
...
export default class GameServer {
        ...
        switch (msg.type) {
            case 'join':
                if (Object.keys(this.state.players).length >= 4) {
                    sender.send(JSON.stringify({ type: 'error', payload: 'Room is full (max 4 players)' }));
                    return;
                }
                this.state.players[sender.id] = {
                    name: msg.payload?.name || `Player ${Object.keys(this.state.players).length + 1}`,
                    joinedAt: Date.now(),
                };
                this.broadcast();
                break;
    ...
    startGame() {
        ...
        this.timerInterval = setInterval(() => {
            ...
            this.state.progress = Math.min(100, ((GAME_DURATION - this.state.timeRemaining) / GAME_DURATION) * 100);
        ...
        if (this.state.activeBugs.length < MAX_ACTIVE_BUGS) {
            try {
                ...
                const bug = {
                    id: bugId,
                    code: code.trim(),
                    title: topic.replace('Write a function that ', '').replace('Write a class that ', ''),
                    spawnedAt: Date.now(),
                };
    ...
    async handleSubmitFix(payload, sender) {
        ...
        if (bugIndex === -1) {
            sender.send(JSON.stringify({ type: 'fix_result', payload: { bugId, fixed: false, explanation: 'Bug not found or already resolved.' } }));
            return;
        }
        ...
        try {
            ...
            try {
                ...
                result = jsonMatch ? JSON.parse(jsonMatch[0]) : { fixed: false, explanation: 'Could not parse validation result.' };
            } catch {
                result = { fixed: false, explanation: 'Could not parse validation result.' };
            }
            ...
            if (result.fixed) {
                ...
                this.state.bugHistory.push({
                    ...
                    title: bug.title,
                    status: 'resolved',
                    resolvedBy: this.state.players[sender.id]?.name || sender.id,
                    fixedCode: code,
                });
            ...
            this.room.broadcast(JSON.stringify({
                type: 'fix_result',
                payload: { bugId, ...result, submittedBy: this.state.players[sender.id]?.name || sender.id }
            }));
            this.broadcast();
        } catch (err) {
            console.error('Fix validation failed:', err);
            sender.send(JSON.stringify({ type: 'fix_result', payload: { bugId, fixed: false, explanation: 'Validation service error.' } }));
        }
    }
    ...
    endGame() {
        ...
        // Calculate score
        const unresolvedCount = this.state.bugHistory.filter(b => b.status === 'unresolved').length;
